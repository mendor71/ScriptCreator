    ...
    function load_data() {
        $.ajax({
            ...
            , success: function (data) {
                var body = document.getElementById('api_table');
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    th_params.setAttribute("width","300px");
                    ...
                    methods.sort(function (c_obj, o_obj) {
                        if (c_obj.method === 'GET' && o_obj.method !== 'GET') return -1;
                        if (c_obj.method !== 'GET' && o_obj.method === 'GET') return 1;
                        if (c_obj.method === 'POST' && (o_obj.method === 'PUT' || o_obj.method === 'DELETE')) return -1;
                        if (c_obj.method === 'PUT' && o_obj.method === 'DELETE') return -1;
                        if (c_obj.method === 'PUT' && o_obj.method === 'POST') return 1;
                        if (c_obj.method === 'DELETE') return 1;
                    ...
                    for (var n = 0; n < methods.length; n++) {
                        ...
                        td_url.appendChild(document.createTextNode(methods[n].url));

                        var td_action = document.createElement("td");
                        td_action.appendChild(document.createTextNode(methods[n].actionName));

                        var td_method = document.createElement("td");
                        td_method.appendChild(document.createTextNode(methods[n].method));

                        var td_params = document.createElement("td");
                        td_params.appendChild(document.createTextNode(methods[n].params));
